...
class Database:
    ...
    def save_tournament_in_db(self, serialized_tournament, tournament_id):
        ...
        self.tournaments_table.upsert(
            serialized_tournament, self.query.tournament_id == tournament_id
        )
    ...
    def load_player_from_id(self, player_id):
        ...
        """
        player = self.players_table.search(self.query.p_id == player_id)[0]
    ...
    def load_players_from_tournament_id(self, tournament_id):
        ...
        """
        tournament_players = []
        tournament = self.tournaments_table.search(
            self.query.tournament_id == tournament_id
        )[0]
        tournament_players_id = tournament["tournament_players_id"]
        for p_id in tournament_players_id:
            tournament_players.append(self.load_player_from_id(p_id))
    ...
    def load_ongoing_tournaments(self):
        ...
        unfinished_tournament = self.tournaments_table.search(
            self.query.end_date == False
        )
    ...
    def deserialize_player_info(serialized_player):
        player = Player(
            ...
            tournament_points=serialized_player["tournament_points"],
            p_id=serialized_player["p_id"],
